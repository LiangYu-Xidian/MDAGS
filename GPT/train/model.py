...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.num_props:
            ...
            self.prop_nn = nn.Linear(1900, config.n_embd)
    ...
    def forward(self, idx, targets=None, prop = None, scaffold = None):
        ...
        ########################################################################################################
        x = self.drop(token_embeddings + position_embeddings + type_embeddings)         #drop函数里的三个变量是什么意思
        #我做的改动
        if self.config.num_props:
            type_embd = self.type_emb(torch.zeros((b, 1), dtype = torch.long, device = idx.device))
            if self.config.num_props == 1:
                print(prop.shape)
                p = self.prop_nn(prop)    # for single property
            else:
